...
### 5.3 環境変数の設定
- [ ] Vercel Dashboardで環境変数追加
  - [ ] `NEXT_PUBLIC_SITE_URL`（本番URL）
  - [ ] `NEXT_PUBLIC_SITE_NAME`
  - [ ] `NEXT_PUBLIC_GA_ID`（Google Analytics、オプション）
- [ ] Production環境に設定
- [ ] Preview環境に設定（必要に応じて）

### 5.4 初回デプロイ
- [ ] Vercel Dashboardから「Deploy」実行
- [ ] ビルドログの確認
- [ ] デプロイ成功の確認
- [ ] デプロイURLにアクセス
...
**次に実行すべきタスク**:
1. Next.js 15プロジェクト作成（`pnpm create next-app@latest`）
2. 基本設定ファイルの調整
3. shadcn/uiの導入

**開始コマンド**:
```bash
pnpm create next-app@latest microblog --typescript --tailwind --app --eslint
cd microblog
```
